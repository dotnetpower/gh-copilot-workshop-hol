...
:::info Strangler Fig 패턴이란?
Strangler Fig 패턴은 기존 레거시 시스템을 한 번에 교체하지 않고, **점진적으로 새로운 시스템으로 대체**해 나가는 마이크로서비스 전환(Modernization) 아키텍처 패턴입니다.

**이름의 유래**  
**Strangler Fig(기생 무화과)** 라는 나무에서 유래했는데, 이 나무는 기존 나무를 감싸며 자라다가 결국 원래 나무를 대체하는 생태를 보입니다.

**핵심 개념**  
레거시 시스템 앞단에 새로운 시스템을 두고, **기능 단위로 하나씩 교체**하면서 최종적으로 레거시를 제거하는 패턴입니다.

```mermaid
graph LR
    A[클라이언트] --> B[새로운 시스템<br/>Facade/Gateway]
    B -->|기능 A| C[새로운 서비스 A]
    B -->|기능 B| C
    B -->|기능 C, D, E| D[레거시 시스템]
...
**장점:**
- ✅ 위험 최소화: 전체 시스템을 한 번에 교체하지 않음
- ✅ 점진적 검증: 각 기능별로 테스트 및 검증 가능
- ✅ 비즈니스 연속성: 서비스 중단 없이 전환
- ✅ 롤백 용이: 문제 발생 시 레거시로 즉시 복귀
:::
...
:::info Extract Method 패턴이란?
Extract Method 패턴은 길고 복잡한 메서드에서 **특정 책임(로직)을 분리**해, 의미 있는 이름의 새로운 메서드로 추출하는 리팩토링 패턴입니다.

👉 **객체지향 리팩토링의 가장 기본이면서도 가장 효과적인 기법**입니다.

**핵심 한 줄 요약**  
"한 메서드는 한 가지 일만 하게 만든다"를 실현하는 리팩토링 패턴

**주요 이점:**
- ✅ 가독성 향상: 메서드 이름이 코드의 의도를 명확히 표현
- ✅ 재사용성: 추출된 메서드를 다른 곳에서도 활용 가능
- ✅ 테스트 용이: 작은 단위로 나뉘어 단위 테스트 작성이 쉬움
- ✅ 유지보수성: 변경 시 영향 범위가 명확해짐

**적용 시점:**
- 메서드가 20줄 이상일 때
- 주석이 필요한 코드 블록이 있을 때
- 중복된 코드가 보일 때
- 메서드 내 추상화 수준이 섞여 있을 때
:::
